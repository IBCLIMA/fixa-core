...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lineasHtml = lineas.map((l) => {
      ...
      return `<tr><td style="padding:8px;border-bottom:1px solid #eee">${l.tipo === "mano_obra" ? "M.O." : l.tipo === "recambio" ? "Recambio" : "Otros"}</td><td style="padding:8px;border-bottom:1px solid #eee">${esc(l.descripcion)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${qty}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${price.toFixed(2)}€</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${base.toFixed(2)}€</td></tr>`;
    }).join("");
