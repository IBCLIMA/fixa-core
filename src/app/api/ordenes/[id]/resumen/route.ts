...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const lineasHtml = lineas.map((l) => {
      ...
      return `<tr><td style="padding:8px;border-bottom:1px solid #eee">${l.tipo === "mano_obra" ? "M.O." : l.tipo === "recambio" ? "Recambio" : "Otros"}</td><td style="padding:8px;border-bottom:1px solid #eee">${esc(l.descripcion)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${qty}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${formatMoney(price)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">${formatMoney(base)}</td></tr>`;
    }).join("");

    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>OR-${esc(orden.numero)} — ${esc(orden.tallerNombre)}</title><style>body{font-family:-apple-system,sans-serif;max-width:600px;margin:0 auto;padding:20px;color:#1c1917}h1{font-size:24px;margin:0}table{width:100%;border-collapse:collapse;margin:16px 0}th{text-align:left;padding:8px;border-bottom:2px solid #e7e5e4;font-size:12px;text-transform:uppercase;color:#78716c}td{font-size:14px}.total{font-size:18px;font-weight:bold}.header{display:flex;justify-content:space-between;margin-bottom:24px}.badge{display:inline-block;padding:4px 12px;border-radius:999px;font-size:12px;font-weight:bold;background:#f5f5f4;color:#57534e}</style></head><body>
    <div class="header"><div><h1>OR-${esc(orden.numero)}</h1><p style="color:#78716c;margin:4px 0">${esc(orden.tallerNombre) || "FIXA"}</p>${orden.tallerTelefono ? `<p style="color:#78716c;margin:0;font-size:14px">${esc(orden.tallerTelefono)}</p>` : ""}${orden.tallerDireccion ? `<p style="color:#78716c;margin:0;font-size:14px">${esc(orden.tallerDireccion)}</p>` : ""}</div><div style="text-align:right"><span class="badge">${esc(orden.estado)}</span><p style="margin:8px 0 0;font-size:14px;color:#78716c">${new Date(orden.fechaEntrada).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}</p></div></div>
    <hr style="border:none;border-top:1px solid #e7e5e4">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0"><div><p style="font-size:12px;color:#78716c;margin:0">VEHÍCULO</p><p style="font-size:18px;font-weight:bold;margin:4px 0;letter-spacing:2px">${esc(orden.matricula)}</p><p style="color:#78716c;margin:0">${esc(orden.marca)} ${esc(orden.modelo)}</p>${orden.kmEntrada ? `<p style="color:#78716c;margin:0;font-size:14px">${esc(orden.kmEntrada.toLocaleString("es-ES"))} km</p>` : ""}</div><div><p style="font-size:12px;color:#78716c;margin:0">CLIENTE</p><p style="font-weight:bold;margin:4px 0">${esc(orden.clienteNombre)}</p>${orden.clienteTelefono ? `<p style="color:#78716c;margin:0;font-size:14px">${esc(orden.clienteTelefono)}</p>` : ""}</div></div>
