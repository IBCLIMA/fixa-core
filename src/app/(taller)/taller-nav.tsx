...
const allMoreTabs = [
  ...
  { name: "Importar datos", href: "/importar", icon: Upload, roles: ["admin", "mecanico", "recepcion"] },
  { name: "Primeros pasos", href: "/primeros-pasos", icon: BookOpen, roles: ["admin", "mecanico", "recepcion"] },
  { name: "Admin", href: "/admin", icon: Shield, roles: ["admin"] },
  { name: "Ayuda", href: "/ayuda", icon: HelpCircle, roles: ["admin", "mecanico", "recepcion"] },
...
export function TallerNav({ children, rol }: { children: React.ReactNode; rol: RolUsuario }) {
  ...
  const moreTabs = allMoreTabs.filter((tab) => tab.roles.includes(rol));
  const allSidebarTabs = [...mainTabs, ...moreTabs];
