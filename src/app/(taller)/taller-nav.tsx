...
const gestionNav = [
  ...
  { name: "Facturación", href: "/facturacion", icon: Receipt, roles: ["admin", "recepcion"] },
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col antialiased" style={{ background: "linear-gradient(180deg, #faf9f7 0%, #f5f3f0 100%)" }}>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-1">
        <aside className="hidden lg:flex w-56 shrink-0 flex-col border-r border-stone-200/60 bg-white/50 px-3 py-4">
          {/* Main section */}
          <div className="space-y-1">
            {mainNav.map((tab) => (
              <SidebarLink key={tab.href} tab={tab} active={isActive(tab.href)} />
            ))}
          </div>
          ...
          {filteredGestion.length > 0 && (
            <>
              <div className="my-3 border-t border-stone-200/40" />
              <p className="px-3 mb-2 text-[10px] font-bold text-stone-400 uppercase tracking-wider">Gestión</p>
              <div className="space-y-1">
          ...
          {filteredAdmin.length > 0 && (
            <>
              <div className="my-3 border-t border-stone-200/40" />
              <p className="px-3 mb-2 text-[10px] font-bold text-stone-400 uppercase tracking-wider">Ajustes</p>
              <div className="space-y-1">
          ...
          <div className="mt-auto pt-3 border-t border-stone-200/40">
            <SidebarLink tab={{ name: "Ayuda", href: "/ayuda", icon: HelpCircle }} active={isActive("/ayuda")} />
          </div>
        </aside>
...
function SidebarLink({ tab, active }: { tab: { name: string; href: string; icon: any }; active: boolean }) {
  return (
    <Link
      href={tab.href}
      className={cn(
