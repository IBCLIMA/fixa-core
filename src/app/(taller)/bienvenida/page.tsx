        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {[0, 1, 2].map((i) => (
            <div key={i} className={`h-2 rounded-full transition-all duration-500 ${
              i === paso ? "w-10 bg-gradient-to-r from-brand-500 to-brand-600" :
              i < paso ? "w-3 bg-brand-500" : "w-3 bg-stone-200"
            }`} />
          ))}
        </div>
        ...
        {paso === 0 && (
          <div className="text-center space-y-8 animate-in fade-in duration-500">
            ...
            <div className="relative inline-block">
              <div className="absolute -inset-4 rounded-3xl bg-gradient-to-br from-brand-300/20 to-brand-500/10 blur-2xl" />
            ...
            <div>
              <h1 className="text-4xl font-extrabold tracking-tight text-stone-900">
                Se acabó el caos.
                <br />
                <span className="text-brand-600">Tu taller, organizado.</span>
              </h1>
            ...
            <div className="space-y-3 text-left">
              <p className="text-xs font-bold text-brand-600 uppercase tracking-wider text-center mb-4">A partir de hoy</p>
              {[
                { icon: Zap, text: "Matrícula y listo. Orden creada en 10 segundos.", color: "text-brand-500 bg-brand-50" },
            ...
            <div className="space-y-3 pt-2">
              <Button onClick={() => setPaso(1)} size="lg" className="rounded-full w-full h-14 font-bold text-base bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 shadow-lg shadow-brand-500/25">
        ...
        {paso === 1 && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="text-center">
              <div className="flex h-12 w-12 mx-auto items-center justify-center rounded-2xl bg-gradient-to-br from-brand-500 to-brand-600 shadow-lg shadow-brand-500/20 mb-4">
            ...
            <div className="flex items-start gap-3 rounded-2xl bg-white/80 border border-stone-200/40 p-4 shadow-sm">
              <input
                ...
                className="mt-0.5 h-5 w-5 rounded border-stone-300 text-brand-500 focus:ring-brand-500 cursor-pointer accent-brand-500"
              />
              <label htmlFor="dpa-check" className="text-sm text-stone-600 cursor-pointer leading-snug">
                ...
                <Link href="/dpa" target="_blank" className="text-brand-600 underline underline-offset-2 font-medium text-xs">
                  Ver condiciones
                </Link>
              </label>
            </div>
            ...
            <div
              ...
              className={`flex items-start gap-4 rounded-2xl p-5 cursor-pointer transition-all duration-200 ${
                newsletterConsent
                  ...
                  : "bg-white/80 border-2 border-stone-200/60 hover:border-brand-200 hover:bg-brand-50/50"
              }`}
            >
              <input
                type="checkbox"
                ...
                className="mt-1 h-5 w-5 rounded border-stone-300 text-brand-500 focus:ring-brand-500 cursor-pointer accent-brand-500"
              />
              <div>
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={guardarDatos} disabled={loading || !dpaAceptado} className="rounded-full flex-1 h-12 font-bold text-base bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 shadow-lg shadow-brand-500/20">
          ...
          <div className="text-center space-y-8 animate-in fade-in duration-500">
            ...
            <div className="space-y-3 text-left">
              {[
                ...
                { icon: ClipboardList, text: "Crea tu primera orden de trabajo", href: "/", color: "text-brand-500 bg-brand-50" },
                ...
                <Link key={item.text} href={item.href} onClick={() => localStorage.setItem("fixa-onboarding-completado", "true")} className="flex items-center gap-4 rounded-2xl bg-white/80 backdrop-blur-sm border border-stone-200/40 p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
                  ...
                  <ArrowRight className="h-4 w-4 text-stone-300 group-hover:text-brand-500 group-hover:translate-x-1 transition-all" />
                </Link>
              ))}
            </div>
            ...
            {/* Main CTA */}
            <div className="space-y-3 pt-2">
              <Button onClick={irAlPanel} size="lg" className="rounded-full w-full h-14 font-bold text-base bg-gradient-to-r from-brand-500 to-brand-600 hover:from-brand-600 hover:to-brand-700 shadow-lg shadow-brand-500/25">
              ...
              <div className="flex items-center justify-center gap-4 text-xs text-stone-400">
                ...
                <span className="flex items-center gap-1"><Star className="h-3 w-3 text-brand-500" />Soporte por WhatsApp</span>
              </div>
            </div>
          </div>
        )}
