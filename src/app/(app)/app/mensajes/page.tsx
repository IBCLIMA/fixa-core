    ...
    <div className="space-y-6 pb-4">
      ...
      {clientes.length === 0 ? (
        ...
        <>
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground">Cliente</Label>
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground">Mensaje rápido</Label>
            <div className="grid grid-cols-2 gap-2">
              {plantillas.map((p) => (
                <button key={p.id} className="flex items-center gap-2.5 rounded-xl border border-border/60 bg-card p-3.5 text-left text-xs font-semibold active:bg-accent hover:bg-accent transition-colors" onClick={() => aplicarPlantilla(p.id)}>
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground">Mensaje</Label>
          ...
          {cliente && mensaje && (
            <div className="rounded-xl bg-green-500/5 border border-green-500/15 p-4">
              <p className="mb-1.5 text-[11px] font-bold text-green-500 uppercase tracking-widest">Vista previa</p>
              <p className="text-sm text-foreground/80 leading-relaxed">{mensaje}</p>
              <p className="mt-2 text-[11px] text-muted-foreground">Se abrirá WhatsApp con este mensaje para {cliente.nombre}</p>
            </div>
          )}
      ...
      {registro.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs font-bold uppercase tracking-[0.15em] text-muted-foreground">Últimos mensajes preparados</p>
          <div className="rounded-2xl border border-border/60 bg-card divide-y divide-border/40">
            {registro.slice(0, 8).map((r) => (
              <div key={r.id} className="flex items-center justify-between px-4 py-3">
                ...
                <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground shrink-0 tabular-nums">
          ...
          <p className="text-[11px] text-muted-foreground text-center">Solo registra que el mensaje se preparó, no si se envió</p>
        </div>
      )}
    </div>
  );
